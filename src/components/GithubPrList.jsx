...
const GithubPRList = ({ fetchForCollection }) => {
    const [pullRequests, setPullRequests] = useState([]);
    const [page, setPage] = useState(0);
    const rowsPerPage = 9;
    const filterUserLogins = ['<GITHUB-LOGINS>'];
    ...
    useEffect(() => {
        if (fetchForCollection) {
            const fetchFilteredPullRequests = async () => {
                try {
                    const response = await fetch(`https://api.github.com/repos/ansible-collections/${fetchForCollection}/pulls?state=open`, {
                        headers: {
    ...
    const fetchReviews = async (pullRequestsData) => {
        const reviewsData = await Promise.all(pullRequestsData.map(async pr => {
            const response = await fetch(`https://api.github.com/repos/ansible-collections/${fetchForCollection}/pulls/${pr.number}/reviews`, {
                headers: {
                    Authorization: `token ${accessToken}`
                }
            });
